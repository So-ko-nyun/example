    ...
    while True:
        ...
        for post in posts:
            ...
            if title_tag:
                ...
                #ID와 업데이트 시간의 제목 부분
                post_title_id = div_id_date[0].find('span', class_='metaDataKey su__font-bold su__color-blue su__mr-2 su__rtlmr-0 su__rtlml-2 su__font-12')
                post_title_date = div_id_date[3].find('span', class_='metaDataKey su__font-bold su__color-blue su__mr-2 su__rtlmr-0 su__rtlml-2 su__font-12')
                    ...
                    if date_check_tag:
                        ...
                        try:
                            ...
                            if is_within_date_range(post_date,start_date,end_date):
                                posts_urls.append({'url': href, "id": post_id, 'date': post_date, 'title': title})
...
# crawl_posts()에 KB게시글이 모여있는 url, 시작날짜, 끝날짜 범위를 주면 미리보기뷰에서 날짜데이터를 수집해서 범위에 맞는 날짜에 속한 게시글들의 url을 딕셔너리로 반환합니다.
# 그 후 받은 url이 담긴 posts_urls리스트를 for문을 통해 전부 접속하여 내부에 있는 세부내역들을 extra_data_from_webpage()함수를 통해 크롤링해서 딕셔너리 형태로 반환해줍니다.
# 세부내역이 들어있는 딕셔너리를 기존의 json파일과 비교하여 중복된 경우는 제외하고 데이터를 합쳐서 저장합니다. 
...
def crawl_posts(base_url, start_date_str, end_date_str):
    ...
    for post in posts_urls:
        ...
        url = post.get('url')
        date = post.get('date')
        if extract_data_from_webpage(url, date) is not None:
            all_posts_details.append(extract_data_from_webpage(url, date))
        else:
            print("이 페이지는 없는 페이지입니다.", url)
